function Offspring = GA(Parent,GA)
...
%% Parameter setting
[proC,disC,proM,disM] = deal(GA.proC,GA.disC,GA.proM,GA.disM);

if any(size(Parent)>1)
    Parent          = reshape(extractfield(Parent,'Position'),[],size(Parent,1))';
else
...
% Polynomial mutation
Lower = repmat(GA.lower,2*N,1);
Upper = repmat(GA.upper,2*N,1);
